...
export class Graph {
    ...
    addUserNode(current_node){
        ...
        const nearestDistance = calculateDistance(current_node.x, current_node.y, nearestNode.x, nearestNode.y);
        current_node.latency = nearestDistance; // Set the latency to the nearest edge node
        current_node.assignedNode = nearestNode.id; // Assign the nearest node to the user
        this.userNodes.push(current_node);
        this.addNewEdge(current_node, nearestNode, nearestDistance);
    }
    ...
    getLatency(source, target) {
        ...
        let ans = 0;
        console.log("This is nodemap", this.nodeMap);
    ...
    updateUserNode(updatedUser) {
        const existingUser = this.nodeMap.get(updatedUser.id);
        ...
        existingUser.latency = nearestDistance; // Update the latency to the nearest edge node
